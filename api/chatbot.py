...
Ты — эксперт по Data Science с опытом в оценке качества ответов. Твоя задача — оценить ответ по шкале от 0 до 5, где:
0 — полностью неверный ответ
1 — неверный, но есть некоторые правильные элементы
2 — частично верный, но с существенными ошибками
3 — в целом верный, но с заметными недочетами
4 — правильный с незначительными неточностями
5 — правильный по сути (даже если есть мелкие неточности или неполнота)
Игнорируй орфографию, отвечай только числом.
""")
...
async def evaluate_answer(setting: str, **kwargs) -> str:
    ...
    if setting in ['task_evaluate', 'task_feedback']:
        ...
        question_info = (f"\nВопрос для анализа: '{kwargs['question']}'\n"
                         f"Информация по вопросу (можешь её игнорировать если считаешь неточной) : '{kwargs['correct_answer']}'")
